...
const UserPreferencesForm = () => {
    // New state to match the Pydantic model
    const [formData, setFormData] = useState({
        travel_type: 'Leisure',
        total_budget: 60000,
        group_type: 'couple',
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            no_of_people: parseInt(formData.no_of_people),
            duration: parseInt(formData.duration),
            // The backend now handles the budget_range split, so we don't send it from here
            budget_range: null 
        };
                            ...
                            <form onSubmit={handleSubmit} className="preferences-form">
                                <div className="form-grid">
                                    <motion.div variants={fieldVariants} className="form-group">
                                        ...
                                        <select id="travel_type" name="travel_type" value={formData.travel_type} onChange={handleChange}>
                                            <option>Leisure</option><option>Business</option><option>Adventure</option><option>Romantic</option><option>Family</option>
                                        </select>
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group">
                                        <label htmlFor="total_budget">Total Budget (₹)</label>
                                        <input id="total_budget" type="number" name="total_budget" value={formData.total_budget} onChange={handleChange} placeholder="e.g., 60000" min="1000" step="1000" required />
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group form-group-full">
                                        <label htmlFor="interests">Interests</label>
                                        <input id="interests" type="text" name="interests" value={formData.interests} onChange={handleChange} placeholder="e.g., mountains, trekking, culture" required />
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group">
                                        <label htmlFor="duration">Trip Duration (days): {formData.duration}</label>
                                        <input id="duration" type="range" name="duration" value={formData.duration} onChange={handleChange} min="1" max="45" />
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group">
                                        <label htmlFor="no_of_people">Number of People: {formData.no_of_people}</label>
                                        <input id="no_of_people" type="range" name="no_of_people" value={formData.no_of_people} onChange={handleChange} min="1" max="30" />
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group">
                                        <label htmlFor="group_type">Group Type</label>
                                        <input id="group_type" type="text" name="group_type" value={formData.group_type} onChange={handleChange} placeholder="e.g., couple, friends, family" required />
                                    </motion.div>
                                    <motion.div variants={fieldVariants} className="form-group">
                                     ...
                                     <motion.div variants={fieldVariants} className="form-group form-group-full">
                                        ...
                                        <select id="planning_style" name="planning_style" value={formData.planning_style} onChange={handleChange}>
                                            <option>Not specified</option><option>Holiday based</option><option>Season based</option>
                                        </select>
                                    </motion.div>
                                </div>
                                <motion.button variants={fieldVariants} type="submit" className="form-submit-btn" disabled={loading}>
